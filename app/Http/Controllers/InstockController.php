...
class InstockController extends Controller
{
    public function index(Request $request)
    {
       ...
       $instocks = Warehouse::join('shelf_numbers', 'warehouses.id', '=', 'shelf_numbers.warehouse_id')
                            ->join('shelves', 'warehouses.id', '=', 'shelves.warehouse_id')
                            ...
                            ->where(function ($query) use ($request) {
                                if ($request->shelf_num_id) {
                                    $query->where('shelf_numbers.id', $request->shelf_num_id);
                                }
                            })
                            ->select(
                                'products.code_id',
                                'shelf_numbers.id as shelf_number_id',
                            )
                            ->groupBy(
                                'products.code_id',
                                'shelf_numbers.id',
    ...
    public function export($instocks, $request_from, $request_to)
    {
        for ($i = 0; $i < count($instocks); $i++) {
            ...
            $instocks[$i] = [
                "No" => $i+1,
                'Warehouse' => $shelf_num->warehouse->name,
                'Shelf' => $shelf_num->shelf->name,
                'ShelfNumber' => $shelf_num->name,
