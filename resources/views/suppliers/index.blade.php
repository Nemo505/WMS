      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
             {{-- form --}}
             @php
                $s_suppliers =  App\Models\Supplier::get();
            @endphp
                  ...
                  <div class="col-5">
                    <div class="form-group">
                      ...
                      <select id='supplier' name="supplier" class=" form-control">
                        ...
                        @foreach ($s_suppliers as $s_supplier)
                          @if (isset($_REQUEST['supplier']))
                              @if ($s_supplier->id == $_REQUEST['supplier'])
                                  <option value="{{ $s_supplier->id }}" selected>{{ $s_supplier->name }}</option>
                              @else
                                  <option value="{{ $s_supplier->id }}">{{ $s_supplier->name }}</option>
                              @endif
                          @else
                              <option value="{{ $s_supplier->id }}">{{ $s_supplier->name }}</option> 
                          @endif
                      ...
                      <select id='phone' name="phone" class=" form-control">
                        ...
                        @foreach ($s_suppliers as $s_supplier)
                          @if (isset($_REQUEST['phone']))
                              @if ($s_supplier->phone == $_REQUEST['phone'])
                                  <option value="{{ $s_supplier->phone }}" selected>{{ $s_supplier->phone }}</option>
                              @else
                                  <option value="{{ $s_supplier->phone }}">{{ $s_supplier->phone }}</option>
                              @endif
                          @else
                              <option value="{{ $s_supplier->phone }}">{{ $s_supplier->phone }}</option> 
                          @endif
          ...
          <div class="card-body" style="overflow-x: scroll;">
            <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  @php
                    ++$i;
                    $c_user = \App\Models\User::find($supplier->created_by);
                    $u_user = \App\Models\User::find($supplier->updated_by);

                    $check_delete = Auth::user()->hasPermissionTo('delete_supplier');
                  @endphp
                    <tr>
                        <td>{{  $i }}</td>
                        <td class="name_{{ $supplier->id }}">{{ $supplier->name }}</td>
                        <td class="phone_{{ $supplier->id }}">{{ $supplier->phone }}</td>
                        <td class="email_{{ $supplier->id }}">{{ $supplier->email }}</td>
                        <td class="address_{{ $supplier->id  }}">{{ $supplier->address }}</td>
                        <td class="sos_{{ $supplier->id  }}">{{ $supplier->emergency }}</td>
                        ...
                        <td>
                            <div class="d-flex justify-content-around"> 
                                <a href="{{ route('suppliers.edit', ["id" => $supplier->id])}}" class="" >
                                    <i class="far fa-edit" style="color: rgb(221, 142, 40)"></i>
                                </a>

                                @if ($check_delete == true)
                                  <a href="" data-toggle="modal" data-target="#del-modal" id="{{ $supplier->id }}" class="del_class">
    ...
    <div class="modal fade" id="del-modal">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <form method="GET" action="{{ route('suppliers.delete') }}">
              <div class="modal-body">
                  ...
                  <div class="row mx-4">
                    <div class="col-6">
                        <label for="del_name">supplier Name :</label>
                    </div>
                    <div class="col-6">
                        <p id="del_name"></p>
                    </div>
