...
@section('title', 'Issue Return History List')
...
@section('buttons')
    <a href="{{ route("issue_returns.index")}}" style="color:rgb(136, 32, 139)">
            ...
            <table id="example2" class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                @php
                  ...
                  $issue = \App\Models\Issue::find($history->issue_id); 
                  $new_issue = \App\Models\Issue::find($history->new_issue_id); 

                  $code = \App\Models\Product::where('products.id', $issue->product_id)
                                                ->join('codes', 'codes.id', '=', 'products.code_id')
                                                ->join('units', 'units.id', '=', 'products.unit_id')
                                                ->first(['codes.name', 'units.name as unit_name']); 

                  $new_code = \App\Models\Product::where('products.id', $new_issue->product_id)
                                                ->join('codes', 'codes.id', '=', 'products.code_id')
                                                ->join('units', 'units.id', '=', 'products.unit_id')
                                                ->first(['codes.name', 'units.name as unit_name']); 
                ...
                @endphp
                  <tr>
                      <td>{{  $i }}</td>
                      <td>{{ $history->issue_return_date }}</td>
                      <td>{{ $history->new_issue_return_date }}</td>

                      <td>{{ $history->mrr_no }}</td>
                      <td>{{ $history->new_mrr_no }}</td>
